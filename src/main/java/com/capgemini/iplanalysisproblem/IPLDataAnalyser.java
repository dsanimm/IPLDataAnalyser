...
public class IPLDataAnalyser<E> {
	...
	public List<String> TopBattingBowlingAverage(String filePath, String filePath2) throws IOException {
		...
		List<String> BowlersNames = csvToBeanBuilderBowl.parse().stream()
				.filter(l -> !(l.avg.equals("-")))
				.sorted((l1, l2) -> 1 + (int) (Double.parseDouble(l1.avg) - Double.parseDouble(l2.avg)))
				.map(l -> l.player)
				.collect(Collectors.toList());
		List<String> BatterNames = csvToBeanBuilderBat.parse().stream()
				.filter(l -> !(l.average.equals("-")))
				.sorted((l1, l2) -> 1 + (int) (Double.parseDouble(l2.average) - Double.parseDouble(l1.average)))
				.map(l -> l.player).collect(Collectors.toList());
		List<String> allRounders = BatterNames.stream().filter(l -> BowlersNames.contains(l)).limit(5).collect(Collectors.toList());
		return (allRounders);
	}
	public List<String> TopRunsWickets(String filePath, String filePath2) throws IOException {
		...
		List<String> BowlersNames = csvToBeanBuilderBowl.parse().stream()
				.filter(l -> !(l.avg.equals("-")))
				.sorted((l1, l2) -> 1 + (int) ((l1.wkts) - (l2.wkts)))
				.map(l -> l.player)
				.collect(Collectors.toList());
		List<String> BatterNames = csvToBeanBuilderBat.parse().stream()
				.filter(l -> !(l.average.equals("-")))
				.sorted((l1, l2) -> 1 + (int) ((l2.runs) - (l1.runs)))
				.map(l -> l.player).collect(Collectors.toList());
		List<String> topRunsWickets = BatterNames.stream().filter(l -> BowlersNames.contains(l)).limit(5).collect(Collectors.toList());
